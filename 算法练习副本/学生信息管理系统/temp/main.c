...
struct stu *Creat(int n)
{
	...
	for(i=1;i<=n;i++)
	{
		...
		menu_print();
		scanf("%s %s %d %f",p1->name,p1->num,&p1->age,&p1->C,&p1->math,&p1->Eng);
		p1->next=NULL;
		if(i==1)
		{
			head=p2=p1;
...
void WriteData_w(struct stu *head)
{
	...
	while(p!=NULL)
	{
		if(fprintf(fp,"%s %s %d %f %f %f %f\n",p->name,p->num,p->age,p->C,p->math,p->Eng,p->PE)<0)
		{
			printf("数据写入出错\n");
			fclose(fp);
			return;
...
int login()
{
    ...
    passw[p]=0;
    if(strcmp(account,"100")==0 && strcmp(passw,"101")==0)
    {
        file_name();
        head=ReadData();
        menu();
    }
    else if(strcmp(account,"123456")==0 && strcmp(passw,"woshixuesheng")==0)
    {
        file_name();
        head=ReadData();
        menu();

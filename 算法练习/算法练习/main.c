...
int main()
{
    ...
    int t=0;
    for(i=0;i<n;i++) if(a[i]>0) b[t++]=a[i];
...
int dfs(int x,int y)
{
    ...
    for(k=0;k<=3;k++)
    {
        ...
        //判断当前点是否是障碍物或已经在路径中
        if(a[nx][ny]=='#') book[nx][ny]=1;
        if((a[nx][ny]=='.' || a[nx][ny]=='s') && book[nx][ny]==0)
        {
            book[nx][ny]=1;     //标记此点已经走过
            dfs(nx,ny);   //尝试下一个点
            book[nx][ny]=0;     //取消此点标记
        }
    }
